...
function Avatar({ src, name }: { src?: string | null; name?: string | null }) {
  const initials = name
    ...
    : '?';
  return src ? (
    <img src={src} alt={name ?? ''} className="w-14 h-14 rounded-2xl object-cover ring-4 ring-white shadow-md" />
  ) : (
    <div className="w-14 h-14 rounded-2xl bg-[#8B0000] flex items-center justify-center ring-4 ring-white shadow-md">
    ...
    <div className="min-h-screen bg-gray-50 antialiased">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl py-8">
          ...
          >
            ...
            <Link
              href="/workshops"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#8B0000] hover:bg-[#700000] text-white text-sm font-bold rounded-xl transition-all active:scale-95 shadow-sm flex-shrink-0"
            >
              Browse Courses
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl py-10 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Left — Enrolled courses (2/3 width) */}
          <motion.div
            custom={4}
            initial="hidden"
            animate="visible"
            variants={fadeUp}
            className="lg:col-span-2 bg-white rounded-3xl border border-gray-100 shadow-sm p-7"
          >
            ...
            <div className="space-y-4">
              {ENROLLED_COURSES.map((course) => (
                <div
                  key={course.id}
                  className="flex items-start gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-red-200 hover:bg-red-50/40 transition-all group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      <span className="text-xs font-semibold text-[#8B0000] bg-red-50 px-2 py-0.5 rounded-full flex-shrink-0">
                        {course.category}
                      </span>
                    </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <Link
                key={href}
                href={href}
                className="flex flex-col items-center gap-2 p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-red-200 hover:bg-red-50 transition-all text-center group"
              >
