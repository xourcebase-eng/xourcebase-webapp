...
export default function SuccessClient({ searchParams }: SuccessClientProps) {
  ...
  const paymentId = getParam('payment_id') || 'N/A';
  const fullName = getParam('name') || 'Valued Participant';
  const email = getParam('email') || '';
  const phone = getParam('phone') || '-';
  const whatsapp = getParam('whatsapp') || '';
  const currentRole = getParam('role') || '';
  const experience = getParam('experience') || '';
  const coupon = getParam('coupon') || 'None';
