...
function Avatar({ src, name, size = 8 }: { src?: string | null; name?: string | null; size?: number }) {
  ...
  return src ? (
    <img
      src={src}
      alt={name ?? 'User'}
      className={`w-${size} h-${size} rounded-full object-cover ring-2 ring-white`}
    />
  ) : (
    <div className={`w-${size} h-${size} rounded-full bg-[#8B0000] flex items-center justify-center ring-2 ring-white`}>
...
function UserDropdown({ session }: { session: any }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Avatar src={session.user?.image} name={session.user?.name} size={8} />
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="flex h-16 items-center justify-between gap-6">
            ...
            {/* Desktop CTAs — switches based on auth state */}
            ...
            >
              ...
              {isLoggedIn && (
                <div className="flex items-center gap-3 px-5 py-4 bg-red-50 border-b border-red-100">
                  <Avatar src={session?.user?.image} name={session?.user?.name} size={10} />
