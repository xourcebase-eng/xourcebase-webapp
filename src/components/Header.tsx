...
function Avatar({ src, name }: { src?: string | null; name?: string | null }) {
  ...
  return src ? (
    ...
    <div className="w-8 h-8 bg-[#C6FF3D] flex items-center justify-center border-2 border-[#14141A]">
      <span
        className="text-[#14141A] text-xs font-bold"
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <ChevronDown
          className={`w-3.5 h-3.5 text-[#14141A]/50 transition-transform duration-200 ${
            open ? 'rotate-180' : ''
          }`}
        />
      </button>
        ...
        {open && (
          ...
          >
            <div className="px-4 py-3 border-b-2 border-[#14141A]/10">
              ...
              <p className="text-xs text-[#14141A]/40 truncate mt-0.5">
                {session.user?.email}
              </p>
            </div>
            <div className="py-1">
...
export default function Header() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 w-full bg-white transition-shadow duration-300 border-b-2 border-[#14141A] ${
          scrolled ? 'shadow-[0_4px_0_0_rgba(20,20,26,0.06)]' : ''
        }`}
        ...
        {menuOpen && (
          <>
            ...
            >
                ...
                <div className="flex items-center gap-3 px-5 py-4 bg-[#C6FF3D]/15 border-b-2 border-[#14141A] flex-shrink-0">
                  {session.user?.image ? (
                    ...
                    <div className="w-10 h-10 bg-[#C6FF3D] flex items-center justify-center border-2 border-[#14141A] flex-shrink-0">
                      <span
                        className="text-[#14141A] text-sm font-bold"
                        style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                      >
                        {session.user?.name
                          ?.split(' ')
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-[#14141A]/50 truncate">
                      {session.user?.email}
                    </p>
                  </div>
                </div>
              ...
              {/* Footer — sign out only when logged in */}
              {isLoggedIn && (
                <div className="flex-shrink-0 px-4 py-5 border-t-2 border-[#14141A]">
                  <button
                    type="button"
                    onClick={() => signOut({ callbackUrl: '/' })}
